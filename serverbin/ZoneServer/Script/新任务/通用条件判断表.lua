...
通用条件判断表 = {
		...
		if type(condition) == 'table' then
			for k,v in pairs(condition) do
				if(type(v)=='table') then
					skillLevel = GetPlayerData(36,v[1])
					return skillLevel >= v[2]
				end
			end

			if condition[2] ~= nil and type(condition[2])=='number' then
				skillLevel = GetPlayerData(36,condition[1])
				return skillLevel >= condition[2]
			end
		end
	...
	tl = function(condition)
		...
		if type(condition) == 'table' then
			for k,v in pairs(condition) do
				if(type(v)=='table') then
					level = GetPlayerData(37,v[1])
					rfalse("level===="..tostring(level).."v[1]===="..tostring(v[1]))
					return level >= v[2]
				end
			end

			if condition[2] ~= nil and type(condition[2])=='number' then
				level = GetPlayerData(37,condition[1])
				rfalse("level===="..tostring(level).."condition[1]===="..tostring(condition[1]))
