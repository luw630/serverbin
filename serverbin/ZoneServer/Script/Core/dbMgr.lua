...
-- 访问数据表 ： 
--     dbMgr[xxx] 通过SID直接访问某个玩家的表数据空间,如果该玩家不在线,则值为 nil
--     dbMgr.xxx  通过表key直接访问自定义数据,该模式下会必然返回一个有效值！

-- 数据表的存储
-- 自定义数据表：默认数据存放在服务器\dbTables目录下，对应的文件名xxx.dbt
-- 玩家数据表：直接存放于玩家保存数据空间，目前大小只有3200字节
-- 表中的 data 为持久化保存的数据，temp 为临时数据，服务器关了，或玩家下线就没有了

-- 脚本刷新对数据表的影响
-- 服务器程序内置了 OnScriptClosing() OnScriptReseted() 两个方法，在完全刷新脚本时会进行调用
-- OnScriptClosing 目前直接调用 dbMgr.UpdateAll() 将所有数据表保存进服务器内部。。。
-- 在后续的访问过程中，会自动的恢复数据表（包括临时数据也会恢复）

-- 修改数据表内容： 
-- 支持添加、修改、删除
-- 由于修改了表操作的元方法，所以所有的操作均会影响到表和父表的统计数据 __p 父表 __c 当前表下的所有项目数 __s 当前表下的所有项的大小总和

-- 数据表的强制加载或保存：
-- 服务器程序内置 SaveDBTable( k, f, c ) 和 LoadDBTable( k, f ) 两个方法
-- LoadDBTable 通过判断 k 的类型，来决定获取玩家的数据表还是用户自定义数据表，返回的表为普通表，需要通过转换才能变为数据表, f为文件名
-- SaveDBTable k 的使用方法同上，f为

-- 注意，该函数和OnScriptReseted只会在脚本完全刷新的情况下才会调用，也就是现在的命令 sreset 1
...
function dbMgr.UpdateAll() 
    for k,v in pairs( dbMgr ) do
        if ( ttype( v ) == 5 ) then
			--20150130 wk 保存玩家数据修改为SI_savePlayerData
           -- SaveDBTable( k )
			SI_savePlayerData(k)
        end
    end
